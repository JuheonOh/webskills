...
export default function National2016() {
  ...
  const description = (
    <>
      서울시민의 일상의 가치를 높이는 <strong>'아름누리 예술재단'</strong>{" "}
      웹사이트 구축 과제입니다. <br />
      <strong>CSS Skew(18도) 변형</strong>과 그림자 효과를 이용한 입체적인 섹션
      디자인이 특징이며, 라디오 버튼과 체크박스 해킹(Checkbox Hack)을 통해{" "}
      <strong>자바스크립트 없이</strong> 동작하는 슬라이딩 배너와 로그인 박스 등
      고난도 Pure CSS 인터랙션을 구현했습니다.
    </>
  );
  ...
  const task2 = {
    ...
    description:
      "JavaScript 사용이 전면 금지된 환경에서 CSS3만으로 인터랙션을 구현했습니다.",
  ...
  const requirements = {
    ...
    description:
      "웹 표준 준수와 브라우저 호환성(IE, Firefox), 그리고 프레임워크 없는 순수 개발 역량을 평가합니다.",
    list: [
        ...
        <div className="relative flex flex-col gap-10 before:absolute before:top-2 before:bottom-2 before:left-3.75 before:w-0.5 before:bg-black/5">
          ...
          <div className="relative pl-10">
            ...
            <p className="text-body text-sm leading-relaxed">
              상단 우측 <strong>톱니 버튼 클릭 후 로그인</strong>(ID:{" "}
              <code>admin</code> / PW: <code>1234</code>) 하면 관리 기능이
              활성화됩니다. 구를 선택한 상태에서 드롭 영역에
              파일(이미지/동영상)을 놓으면 즉시 반영됩니다.
            </p>
          </div>
  ...
  const task3 = {
    ...
    description: (
      <>
        서울시 25개 자치구 정보를 제공하는 <strong>'인사이드서울'</strong> 웹
        애플리케이션입니다. <br />
        SVG 지도의 좌표 데이터와 JSON 비동기 로딩을 결합하여 인터랙티브한
        대시보드를 구축했으며, Canvas 기반의{" "}
        <strong>실시간 미디어 프로세싱</strong>(동영상 프레임 캡처)과 드래그 앤
        드롭 파일 관리 시스템을 별도의 프레임워크 없이{" "}
        <strong>자바스크립트와 jQuery</strong>만으로 구현했습니다.
      </>
    ),
    tags: [
      "SVG Map Interaction",
