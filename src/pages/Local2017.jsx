...
export default function Local2017() {
  ...
  const description = (
    <>
      한국산업인력공단(HRDKorea) 외국인 교육생들을 위한 서울 여행 가이드 <strong>'Seoul Travel'</strong> 구축 과제입니다. <br />
      가로 <strong>1440px</strong>의 와이드한 레이아웃을 기반으로 <strong>780px, 480px</strong> 반응형 대응력을 평가하며,
      굴림체와 나눔고딕을 혼용한 타이포그래피와 이미지 중심의 히어로 섹션 배치를 통해
      프레임워크 없이 오직 <strong>Pure HTML/CSS</strong>만으로 정보의 가독성과 레이아웃의 안정성을 동시에 확보했습니다.
    </>
  );
  ...
  const requirements = {
    ...
    description: "제공된 디자인을 바탕으로 웹 표준을 준수하며, 프레임워크 없이 순수 CSS만으로 구현해야 합니다.",
    list: [
        ...
        <div className="relative space-y-10 before:absolute before:top-2 before:bottom-2 before:left-[15px] before:w-[2px] before:bg-black/5">
          ...
          <div className="relative pl-10">
            ...
            <p className="text-sm leading-relaxed text-body">
              내비게이션 메뉴 클릭 시 단순 이동이 아닌 <code>animate({"{scrollTop}"})</code>를 활용하여 2초 이내에 해당 섹션으로 부드럽게 도달하도록 구현되었습니다.
            </p>
          </div>
          ...
          <div className="relative pl-10">
            ...
            <h5 className="mb-2 font-bold text-head text-lg tracking-tight">동적 슬라이더 (이미지 강제 삽입)</h5>
            <p className="text-sm leading-relaxed text-body">
              HTML 수정 없이 <code>#slider</code>에 이미지 3장을 스크립트로 추가하고, 1~3초 사이의 전환 효과를 가진 무한 루프 슬라이더를 구축했습니다.
            </p>
          </div>
          ...
          <div className="relative pl-10">
            ...
            <h5 className="mb-2 font-bold text-head text-lg tracking-tight">조건부 Parallax 및 Read More</h5>
            <p className="text-sm leading-relaxed text-body">
              섹션 상단이 브라우저 상단에 닿을 때 이미지를 회전/확대합니다. <code>.hidden-text</code> 영역은 Read More 버튼 클릭 시 슬라이드 다운으로 확장됩니다.
            </p>
          </div>
          ...
          <div className="relative pl-10">
            ...
            <h5 className="mb-2 font-bold text-head text-lg tracking-tight">데이터 매칭 팝업 시스템</h5>
            <p className="text-sm leading-relaxed text-body">
              갤러리 이미지 클릭 시 <code>big_</code> 접두어를 붙여 고해상도 이미지를 로드하고, 메달리스트 클릭 시 JSON 데이터를 매칭한 상세 다이얼로그를 생성합니다.
            </p>
          </div>
        </div>
  ...
  const task3 = {
    ...
    description: (
      <>
        제공된 템플릿의 소스 수정을 일절 배제하고, 오직 <strong>app.js</strong> 파일 하나로
        DOM 조작부터 복잡한 타이밍 기반 애니메이션까지 제어하는 프로젝트입니다.
        실무에서 흔히 접하는 '수정 불가능한 레거시 환경'에서의 개발 역량을 테스트합니다.
      </>
    ),
