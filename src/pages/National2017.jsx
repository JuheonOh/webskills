...
export default function National2017() {
  ...
  const description = (
    <>
      제주도의 수려한 자연경관을 홍보하는 <strong>'JEJU TOURISM'</strong> 웹사이트 구축 과제입니다. <br />
      핑크와 파스텔 톤의 부드러운 색감을 바탕으로, <strong>Perspective 3D Flip</strong> 애니메이션이 적용된 메인 슬라이더와
      자동으로 전환되는 배경 이미지 시스템을 갖추고 있습니다. 라디오 버튼과 체크박스를 활용하여
      섹션 확장 및 비디오 토글 기능을 구현하는 등 <strong>자바스크립트 없는 고난도 인터랙션</strong>의 정수를 보여줍니다.
    </>
  );
        ...
        <div className="relative space-y-10 before:absolute before:top-2 before:bottom-2 before:left-[15px] before:w-[2px] before:bg-black/5">
          ...
          <div className="relative pl-10">
            ...
            <p className="text-sm leading-relaxed text-body">
              별도의 서버 없이 브라우저 내장 DB(<code>WebSQL</code>)를 활용하여 회원 정보,
              게시글, 댓글을 영구 저장합니다. 관리자 계정(<code>admin</code> / <code>1234</code>)으로
              로그인하면 데이터 수정 권한이 부여됩니다.
            </p>
          </div>
          ...
          <div className="relative pl-10">
            ...
            <h5 className="mb-2 font-bold text-head text-lg tracking-tight">지능형 검색 및 오토컴플릿</h5>
            <p className="text-sm leading-relaxed text-body">
              <code>jQuery UI Autocomplete</code>와 연동하여 DB 내의 모든 장소명을 실시간으로
              검색하며, 검색어 및 카테고리 필터 상태를 <code>LocalStorage</code>에
              저장하여 새로고침 후에도 유지되도록 구현했습니다.
            </p>
          </div>
