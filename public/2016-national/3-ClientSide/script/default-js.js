...
function showRouteInformation(element) {
  ...
  if (self.files.length) {
    $('#gallery').html(
      "<li class='loading' style='width:100%; line-height:150px; font-size:15px'>불러오는 중 입니다. (이 문장이 사라지고 나서도 조금의 <b>지연시간</b>이 <b>발생</b> 할 수 있습니다.)</li>",
    )
  }
  ...
  $.each(file, function (key, val) {
    if (val.type == 'video/mp4') {
      ...
      vid.addEventListener('loadeddata', function () {
        ...
        var li = $('<li></li>')
        var playBtn = $(
          "<img src='images/play_btn.png' alt='play_btn.png' title='play_btn.png' class='playBtn' data-key=" +
            key +
            ' />',
        )
        var previewImg = $(
    ...
    switch (splitType[1]) {
      ...
      default:
        error +=
          '(' +
          val.name +
          ') 은(는) 이미지(JPG, PNG) 또는 동영상(MP4) 형식이 아닙니다.\r\n'
        break
    }
  })
...
function showLayerPopup(fileKey) {
  ...
  if (file.type == 'video/mp4') {
    $(
      "<div id='dialog'><video controls autoplay><source src=" +
        file.src +
        ' type=' +
        file.type +
        ' /></video></div>',
