...
var search =
  localStorage.getItem('search') == null ? '' : localStorage.getItem('search')
var scrollTop =
  localStorage.getItem('scrollTop') == null
    ? 0
    ...
    function (tx) {
            ...
            $.each(json['information'], function (key, val) {
              db.transaction(
                function (tx) {
                  ...
                  tx.executeSql(
                    'insert into list (name, info, categories, image, wdate) values (?, ?, ?, ?, ?)',
                    [
                      val['name'],
                      val['info'],
                      val['categories'],
      ...
      tx.executeSql('select rowid from member', [], function (tx, result) {
        if (result.rows.length == 0) {
          db.transaction(function (tx) {
            tx.executeSql(
              'insert into member (userid, pw, username, phone) values (?, ?, ?, ?)',
              ['admin', '1234', '관리자', '01012341234'],
            )
          })
        }
      })
...
function initTrigger() {
  ...
  $('body').on('click', '.modi_view', function () {
    ...
    db.transaction(function (tx) {
      tx.executeSql(
        'select rowid, * from list where rowid=?',
        [lidx],
        function (tx, result) {
          var rs = result.rows[0]
    ...
    $('#file').change(function (e) {
      ...
      if (
        ...
        alert(
          '이미지 형식(JPG, JPEG, PNG, GIF)으로 된 파일만 업로드할 수 잇습니다.',
        )
      }
    })
  ...
  $('#modiFrm').submit(function () {
    ...
    db.transaction(
      function (tx) {
        tx.executeSql(
          'update list set name=?, info=?, image=?, wdate=? where rowid=?',
          [name, info, image, wdate, lidx],
        )
      },
      function (e) {
        console.log(e)
  ...
  $('#loginFrm').submit(function () {
    ...
    db.transaction(
      function (tx) {
        tx.executeSql(
          'select rowid, * from member where userid=? and pw=?',
          [userid, pw],
          function (tx, result) {
            var rs = result.rows[0]
            if (rs) {
  ...
  $(window).scroll(function () {
    ...
    if ($(window).scrollTop() == $(document).height() - $(window).height()) {
      ...
      db.transaction(
        function (tx) {
          tx.executeSql(
            'select rowid, * from list where 1=1 ' + where,
            [],
            function (tx, result) {
  ...
  $('#commentFrm').submit(function () {
    ...
    db.transaction(
      function (tx) {
        tx.executeSql(
          'insert into comment (lidx, midx, comment, wdate) values(?, ?, ?, ?)',
          [lidx, sessionStorage.idx, comment, wdate],
        )
      },
      function (e) {
        console.log(e)
        ...
        function (tx, result) {
          $.each(result.rows, function (key, rs) {
            var html =
              ...
              rs['categories'] +
              '</h3>\
										<div><button type="button" class="more_view" data-idx="' +
              rs['rowid'] +
              '">더 보기</button></div>\
									</div>\
								</div>\
...
function showCommentList(idx) {
  ...
  db.transaction(function (tx) {
    tx.executeSql(
      "select rowid, * from comment where lidx='" +
        idx +
        "' order by rowid desc",
      [],
      function (tx, result) {
        $.each(result.rows, function (key, rs) {
          db.transaction(function (tx2) {
            tx2.executeSql(
              'select rowid, * from member where rowid=?',
              [rs['midx']],
              function (tx2, result2) {
                ...
                if (sessionStorage.idx == rs['midx']) {
                  ul +=
                    '<li><button type="button" class="deleteCommentBtn" data-cidx="' +
                    rs['rowid'] +
                    '">삭제</button></li>'
                }
                ul += '</ul>'
